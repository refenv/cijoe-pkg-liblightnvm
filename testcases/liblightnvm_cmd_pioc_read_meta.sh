...
#
# Passes through a read command with 4K data and 16byte meta payload
#
...
# Create meta
if ! CMD_META_PATH=$(ssh::cmd_output "tempfile -d /tmp -s $CMD_FNAME.meta"); then
  test::fail
fi
if ! ssh::cmd "dd if=/dev/zero of=$CMD_META_PATH bs=16 count=1"; then
  test::fail
fi
